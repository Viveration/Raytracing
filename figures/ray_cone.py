"""Содержит класс, описывающий распространение луча в коническом волокне"""
import numpy as np
...
class Ray_cone:
    ...
    def __init__(self, azimut = 0.0, latitude = 0.0, startpoint = [0.0,0.0,0.0]): #все углы принимаются в радианах
    ...
    def calculate_intersection(self, base_radius, c):
    ...
    def set_angles(self, azimut, latitude):
        ...
        self.vector = [np.sin(latitude)*np.cos(azimut),
                      ...
                      np.cos(latitude)]
        return [latitude, azimut]
    def calculate_trajectory(self, fiber, max_reflection = 1000, angle_elimination = True, output = True):
        ...
        for i in range(1, max_reflection):
            intersection_point = self.calculate_intersection(fiber.base_r, fiber.c)
